...
class IronicReboundAnalyzer:
    ...
    def _extract_concept_logits_at_positions(self, prompt: str, concept: str) -> Dict[int, float]:
        tokens = self._tokenize_prompt(prompt)
        # 1. Call our new, more lenient function
        concept_tokens = self._get_concept_token_ids(concept)
        
        # If the concept is invalid (e.g., not a string), we stop here.
        if not concept_tokens:
            return {}
        ...
        for pos in self.config.temporal_positions:
            if abs(pos) < tokens.shape[1]:
                try:
                    ...
                    # 2. Get the maximum logit across ALL tokens that make up the concept
    ...
    def _get_concept_logit_from_tokens(self, tokens: torch.Tensor, concept: str) -> float:
        ...
        # 2. Return the max logit across all concept tokens at the final position
    ...
    def _get_concept_direction(self, concept: str) -> Optional[torch.Tensor]:
        ...
        # 2. Get the embedding vectors for all tokens and average them.
        # This creates a single "concept direction" that represents the whole word.
        return self.model.W_U[:, concept_tokens].mean(dim=-1)
    
    def _get_concept_token_ids(self, concept: str) -> List[int]:
        """
        Encodes the concept using the model's tokenizer and returns all
        resulting token IDs. This is more flexible than a single-token approach.
        """
        # Ensure the concept is a string to prevent errors
        if not isinstance(concept, str):
            return []
        # Encode the concept with a leading space (standard practice) and return all its tokens
